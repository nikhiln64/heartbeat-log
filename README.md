...
The experiment is complete. `./gradlew redTest` deterministically loses two committed telemetry entries under the pre-KIP-101 rule (seed 5, crash at t=67); the green twin in the default suite replays the IDENTICAL schedule under `EPOCH_BOUNDARY` - Kafka's fix, built here from first principles - and loses nothing. Same seed, same faults, one variable changed: a controlled experiment, not two anecdotes. The standalone election module is in: quorum votes grant the epochs (terms), with Raft's up-to-date check refusing candidates with incomplete logs and votes written durably before any reply - the consensus half of the prompt, in code, tested down to the vote-crash-revote regression. Remaining: the schedule fuzzer (stretch) and panel-prep polish. The git log tells the build story in order.
